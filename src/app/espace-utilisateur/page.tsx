...
export default function EspaceUtilisateur() {
  ...
  /* Vrai pendant qu'on restaure la fiche animal depuis la base de
     données (session locale incomplète) — évite de réafficher le
     formulaire obligatoire alors que les infos existent côté serveur. */
  ...
  useEffect(() => {
    ...
    if (!stored) {
      router.replace("/");
    } else if (!hasPetDetails(stored)) {
      /* Session locale sans fiche animal complète (nouvel appareil,
         stockage effacé…) : on tente de la restaurer depuis la base de
         données du site avant de bloquer l'utilisateur avec le
         formulaire. Si la base n'a rien, le formulaire s'affichera
         comme avant. */
      setRestoringProfile(true);
      hydrateUserFromDb(stored)
        .then((merged) => {
          if (merged) setUser(merged);
        })
        .catch(() => {})
        .finally(() => setRestoringProfile(false));
    }
    checkAndSendAppointmentReminders();
    /* Si le rappel automatique (RDV < 24h) vient de déposer un message
       dans la conversation partagée, on le récupère tout de suite pour
       qu'il apparaisse avec son badge "non lu" sans devoir recharger. */
    if (stored) {
      const resynced = syncMessagesFromShared(loadUser() || stored);
      if (resynced !== stored) setUser(resynced);
    }
  }, [router]);

  /* ─── Synchronisation multi-appareils (base de données du site) ───
     Toutes les 4 s (et à chaque retour sur l'onglet) : récupération de
     l'état partagé — messages de l'équipe, statuts de RDV mis à jour par
     le cabinet, rappels automatiques 24 h déposés côté serveur — puis
     rafraîchissement de l'espace si quelque chose a changé. */
  useEffect(() => {
    return startServerSync(() => {
      const fresh = loadUser();
      if (!fresh) return;
      let u = syncMessagesFromShared(fresh);
      u = syncAppointmentsFromShared(u);
      u = syncReferentFromShared(u);
      setUser(u);
    });
  }, []);
  ...
  /* Scroll lock (drawer, messagerie, recherche) : body figé + scroll
     restauré à la fermeture, à la position exacte et instantanément. */
  ...
  useEffect(() => {
    ...
    const shouldLock = sidebarOpen || messagingOpen || searchOpen;
    if (shouldLock) {
  ...
  }, [sidebarOpen, messagingOpen, searchOpen]);
  ...
  if (!checked || !user || restoringProfile) {
    return (
      ...
      <header className="bg-white shadow-[0_2px_15px_rgba(0,0,0,0.1)] px-3 sm:px-[5%] py-3 sm:py-5 flex items-center justify-between sticky top-0 z-30">
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
          ...
          <div ref={searchRef} className="relative">
            ...
            {/* Desktop dropdown */}
            {searchOpen && (
              <div className="hidden md:block absolute right-0 mt-3 w-[calc(100vw-2rem)] max-w-[420px] max-h-[70vh] overflow-y-auto bg-white rounded-[20px] shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-[#eee] z-50">
                {SearchResultsList}
              </div>
            )}
          ...
          <div className="flex flex-col gap-6 sm:gap-10">
            ...
            <div className="flex flex-col lg:flex-row gap-5 sm:gap-7 items-stretch">
              ...
              {/* Animal card — icon selected in the "fiche animal" step
                  ...
                  main card. */}
              <div className="w-full lg:w-[260px] shrink-0 bg-white p-5 sm:p-7 rounded-[20px] shadow-[0_5px_20px_rgba(0,0,0,0.05)] relative overflow-hidden flex flex-col items-center text-center justify-center gap-3">
                <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-[#2c8c99] via-[#4dd0e1] to-[#2c8c99]" />
                {user.petSpecies ? (
                  <>
