...
/* ─── Fusion des listes admin avec la base du site ───
   La liste des clients/stagiaires du tableau de bord vivait uniquement
   dans le localStorage du navigateur de l'admin : sur un autre appareil,
   les vrais inscrits n'apparaissaient pas. Désormais l'espace admin
   fusionne régulièrement les enregistrements de la base (/api/clients,
   /api/stagiaires) dans sa liste locale — sans jamais écraser des
   champs locaux plus riches, et sans réveiller les notifications. */
...
export function mergeDbClientsIntoAdminList(dbClients: DbClientRow[]) {
  if (typeof window === "undefined" || !Array.isArray(dbClients) || !dbClients.length) return;
  const list = loadAllClients();
  let changed = false;
  dbClients.forEach((db) => {
    if (!db || !db.email) return;
    const idx = list.findIndex((c) => c.email.toLowerCase() === db.email.toLowerCase());
    if (idx < 0) {
      list.push({
        name: db.name,
        email: db.email,
        phone: db.phone || undefined,
        pet: db.pet || undefined,
        petSpecies: db.petSpecies || undefined,
        assignedVet: db.assignedVet || "Dr. Sophie Martin",
        joinedAt: db.joinedAt || new Date().toISOString(),
        appointmentCount: db.appointmentCount || 0,
      });
      changed = true;
    } else {
      const prev = list[idx];
      const next: AdminClientSummary = {
        ...
        pet: db.pet || prev.pet,
        petSpecies: db.petSpecies || prev.petSpecies,
        assignedVet: db.assignedVet || prev.assignedVet,
      };
  ...
  if (changed) saveAllClients(list);
}

export type DbStagiaireRow = {
  name: string;
  email: string;
  phone?: string | null;
  school?: string | null;
...
export function mergeDbStagiairesIntoAdminList(dbStagiaires: DbStagiaireRow[]) {
  if (typeof window === "undefined" || !Array.isArray(dbStagiaires) || !dbStagiaires.length) return;
  const list = loadAllStagiaires();
  let changed = false;
  dbStagiaires.forEach((db) => {
    if (!db || !db.email) return;
    const idx = list.findIndex((s) => s.email.toLowerCase() === db.email.toLowerCase());
    if (idx < 0) {
      list.push({
        name: db.name,
        email: db.email,
        ...
        mentor: db.mentor || "Dr. Sophie Martin",
        startDate: db.startDate,
        endDate: db.endDate,
        status: db.status || "en cours",
      ...
      const prev = list[idx];
      const next: AdminStagiaireSummary = {
        ...prev,
        name: db.name || prev.name,
        phone: db.phone || prev.phone,
        school: db.school || prev.school,
        mentor: db.mentor || prev.mentor,
        progress: typeof db.progress === "number" && db.progress > prev.progress ? db.progress : prev.progress,
      };
      if (JSON.stringify(next) !== JSON.stringify(prev)) {
        list[idx] = next;
        changed = true;
  ...
  if (changed) saveAllStagiaires(list);
}
...
/* ─── Appointment reminder emails ───
   Il n'y a pas de cron : "quand le RDV approche" est vérifié
   opportunistiquement. Le MESSAGE automatique dans la messagerie du
   client, lui, est désormais déposé par le SERVEUR (GET /api/sync →
   readSyncState, garde remindedAppointments) : il part donc même si
   aucun admin n'ouvre son tableau de bord, et sans jamais doubler le
   message. Ici on ne garde que les e-mails, une seule fois par RDV
   grâce à la garde localStorage (par navigateur). */
...
export function checkAndSendAppointmentReminders() {
  ...
  upcoming.forEach((a) => {
    ...
    /* Le message automatique dans la messagerie du client est déposé par
       le serveur (readSyncState) — pas ici, pour éviter les doublons
       entre appareils. */
