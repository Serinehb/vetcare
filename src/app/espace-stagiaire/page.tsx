...
/* ─── (Supprimé) Fausses réponses automatiques de l'équipe ───
   Elles se déclaraient comme de vrais messages "equipe" : côté admin,
   le fil paraissait déjà répondu et le badge des messages non lus ne
   remontait plus. La messagerie reste désormais 100 % réelle. */
...
export default function EspaceStagiaire() {
  ...
  /* ─── Synchronisation multi-appareils (base de données du site) ───
     Récupère les messages de l'équipe (réponse du tuteur, changement de
     mentor…) déposés depuis un autre appareil, et rafraîchit l'espace. */
  useEffect(() => {
    return startServerSync(() => {
      const fresh = loadStagiaire();
      if (!fresh) return;
      setUser(syncStagiaireFromShared(fresh));
    });
  }, []);
  ...
  /* Scroll lock (drawer, messagerie, recherche, fiche ressource) : body
     figé + scroll restauré à la fermeture, à la position exacte. */
  ...
  useEffect(() => {
    /* searchOpen inclus : tant que la barre/le panneau de recherche est
       sortie, la page principale reste bloquée (pas de défilement). */
    const shouldLock = sidebarOpen || messagingOpen || searchOpen || !!selectedResource;
    if (shouldLock) {
  ...
  }, [sidebarOpen, messagingOpen, searchOpen, selectedResource]);
  ...
  const handleSendMessage = (e: FormEvent<HTMLFormElement>) => {
    ...
    /* Plus de fausse réponse automatique : elle apparaissait comme un
       ...
       badge "non lu" côté admin. Les vraies réponses arrivent via la
  ...
  const UserDropdown = (
    <div ref={userMenuRef} className="relative">
      <button
        ...
        className="group flex items-center gap-1.5 sm:gap-2.5 px-2 sm:px-3 py-1.5 rounded-full shadow-[inset_0_0_0_2px_#2c8c99] text-[#2c8c99] font-medium transition-all duration-300 hover:bg-[#2c8c99] hover:text-white"
      >
        ...
        <span className="hidden sm:inline truncate max-w-[120px]">{firstName(user.name)}</span>
        <i className="fa-solid fa-chevron-down text-[10px] ml-0.5 hidden sm:inline" />
      </button>

      {userMenuOpen && (
        <>
          <div className="fixed inset-0 z-40" onClick={() => setUserMenuOpen(false)} />
          <div className="absolute right-0 mt-3 w-[calc(100vw-2rem)] max-w-[280px] bg-white rounded-[20px] shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-[#eee] z-50 overflow-hidden">
  ...
  return (
    <div className="min-h-screen bg-[#f4f7f6] font-[family-name:var(--font-geist-sans)]">
      {/* ── HEADER ── */}
      <header className="bg-white shadow-[0_2px_15px_rgba(0,0,0,0.1)] px-3 sm:px-[5%] py-3 sm:py-5 flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center gap-2 sm:gap-4 min-w-0">
          <button
            type="button"
            onClick={() => setSidebarOpen(true)}
            aria-label="Ouvrir le menu"
            className="lg:hidden w-10 h-10 shrink-0 flex items-center justify-center text-xl text-[#333]"
          >
          ...
          <Link
            href="/"
            className="logo-link text-xl sm:text-2xl font-bold flex items-center gap-2 text-[#333] cursor-pointer shrink-0"
          >
            <i className="fa-solid fa-graduation-cap logo-paw" />
            VetCare
          </Link>
          <span className="hidden md:block text-sm text-[#888] truncate">
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
          ...
          <div ref={searchRef} className="relative">
            ...
            {searchOpen && (
              <div className="hidden md:block absolute right-0 mt-3 w-[calc(100vw-2rem)] max-w-[380px] max-h-[70vh] overflow-y-auto bg-white rounded-[20px] shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-[#eee] z-50">
                {SearchResultsList}
              </div>
            )}

            {searchOpen && (
              <div className="md:hidden fixed left-0 right-0 top-[60px] sm:top-[73px] z-50 bg-white shadow-[0_20px_50px_rgba(0,0,0,0.15)] border-t border-[#eee] max-h-[80vh] overflow-y-auto">
          ...
          <div className="relative">
            ...
            {notifOpen && (
              <>
                ...
                <div className="absolute right-0 mt-3 w-[calc(100vw-2rem)] max-w-[320px] max-sm:fixed max-sm:left-2 max-sm:right-2 max-sm:top-[64px] max-sm:mt-0 max-sm:w-auto max-sm:max-w-none bg-white rounded-[20px] shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-[#eee] z-40 overflow-hidden">
      ...
      {sidebarOpen && (
        ...
        >
          <div
            className="mobile-menu-panel absolute top-0 left-0 h-full w-full max-h-[90vh] overflow-y-auto no-scrollbar bg-white shadow-[0_20px_50px_rgba(0,0,0,0.2)] flex flex-col p-4 sm:p-6 gap-2 rounded-b-[24px]"
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                ...
                onClick={() => setSidebarOpen(false)}
                aria-label="Fermer"
                className="w-9 h-9 shrink-0 rounded-full bg-[#f4f7f6] text-[#888] flex items-center justify-center transition-all duration-300 hover:bg-[#2c8c99] hover:text-white"
              >
      ...
      {/* ── MAIN CONTENT ── */}
      <main className="px-4 sm:px-[5%] py-6 sm:py-10 max-w-[1200px] mx-auto lg:pl-28 lg:pr-28">
        ...
        {tab === "vue" && (
          <div className="flex flex-col gap-6 sm:gap-10">
            {/* ── HERO CARD ── */}
            <div className="flex flex-col lg:flex-row gap-5 sm:gap-7 items-stretch">
              <div className="flex-1 bg-white p-5 sm:p-10 rounded-[20px] shadow-[0_5px_20px_rgba(0,0,0,0.05)] relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-[#2c8c99] via-[#4dd0e1] to-[#2c8c99]" />
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
                  <div className="w-16 h-16 sm:w-[90px] sm:h-[90px] bg-[#2c8c99] rounded-full flex items-center justify-center text-white text-2xl sm:text-3xl font-bold shrink-0 shadow-[0_10px_25px_rgba(44,140,153,0.35)]">
                    {initials(user.name)}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-[#333] mb-2 leading-tight">
                      {greeting()}, {firstName(user.name)}.
                    </h2>
                    <p className="text-sm sm:text-base text-[#666] leading-relaxed max-w-lg">
                      {user.school
                ...
                <div className="flex flex-wrap gap-4 sm:gap-6 mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-[#eee]">
                  <div className="flex items-center gap-2 text-xs sm:text-sm text-[#555]">
              ...
              <div className="w-full lg:w-[260px] shrink-0 bg-white p-5 sm:p-7 rounded-[20px] shadow-[0_5px_20px_rgba(0,0,0,0.05)] relative overflow-hidden flex flex-col text-center gap-3">
                <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-[#2c8c99] via-[#4dd0e1] to-[#2c8c99]" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 sm:gap-7">
              <div className="group bg-white p-5 sm:p-10 rounded-[20px] shadow-[0_5px_20px_rgba(0,0,0,0.05)] transition-all duration-500 hover:[transform:translateY(-0.5rem)] hover:shadow-[0_15px_40px_rgba(0,0,0,0.1)]">
                <div className="w-14 h-14 sm:w-[70px] sm:h-[70px] bg-[rgba(44,140,153,0.1)] rounded-full flex items-center justify-center text-[#2c8c99] text-2xl sm:text-3xl mb-3 sm:mb-4 transition-all duration-500 group-hover:scale-110 group-hover:bg-[#2c8c99] group-hover:text-white">
                  <i className="fa-solid fa-hourglass-end" />
                </div>
                <p className="text-3xl sm:text-4xl font-bold text-[#333] mb-1">{daysInfo.remaining}</p>
                <p className="text-xs sm:text-sm text-[#888] uppercase tracking-wide font-semibold">Jours restants</p>
              </div>
              ...
              <div className="group bg-white p-5 sm:p-10 rounded-[20px] shadow-[0_5px_20px_rgba(0,0,0,0.05)] transition-all duration-500 hover:[transform:translateY(-0.5rem)] hover:shadow-[0_15px_40px_rgba(0,0,0,0.1)]">
                ...
                <p className="text-3xl sm:text-4xl font-bold text-[#333] mb-1">{taskProgress}%</p>
                <p className="text-xs sm:text-sm text-[#888] uppercase tracking-wide font-semibold">Progression tâches</p>
              </div>
              ...
              <div className="group bg-white p-5 sm:p-10 rounded-[20px] shadow-[0_5px_20px_rgba(0,0,0,0.05)] transition-all duration-500 hover:[transform:translateY(-0.5rem)] hover:shadow-[0_15px_40px_rgba(0,0,0,0.1)]">
                ...
                <p className="text-3xl sm:text-4xl font-bold text-[#333] mb-1">{doneTasks}/{totalTasks}</p>
                <p className="text-xs sm:text-sm text-[#888] uppercase tracking-wide font-semibold">Tâches complétées</p>
              </div>
            </div>
