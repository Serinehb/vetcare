...
import { assignVetTeamMember, persistClientToDb } from "./espace-utilisateur/user";
import { addNewClientNotification } from "../lib/vetcare-shared";
...
export default function Home() {
  ...
  const [heroBtnTapped, setHeroBtnTapped] = useState(false);
  const [tappedServiceCard, setTappedServiceCard] = useState<number | null>(
    null
  );
  const [tappedTeamCard, setTappedTeamCard] = useState<number | null>(null);
  ...
  /* Session admin éventuelle : permet d'afficher un bouton "Espace admin"
     sur la page d'accueil. Retourner à l'accueil ne déconnecte JAMAIS —
     la session reste enregistrée et le bouton permet d'y retourner. */
  ...
  const finishSignup = (withPetInfo: boolean) => {
    ...
    /* Enregistre aussi le compte + la fiche animal dans la base de
       données du site, pour ne plus jamais redemander ces infos. */
    void persistClientToDb(finalUser, assignVetTeamMember(finalUser.email).name);
  ...
  const handleModalSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (activeModal === "stage") {
      ...
      /* Enregistrement du stagiaire dans la base du site (persistant,
         visible dans le tableau de bord admin même depuis un autre
         appareil) + notification "Nouvelle inscription stagiaire" dans
         la cloche admin (synchronisée entre appareils). */
      void fetch("/api/stagiaires", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          school,
          motivation,
          mentor: assignVetTeamMember(email).name,
          startDate,
          endDate,
        }),
      }).catch(() => {});
      addNewClientNotification(name, email, "stagiaire");
      sendEmailNotification(
        email,
    ...
    /* Synchronise le compte en base (crée la ligne si elle n'existe pas
       encore, p.ex. compte créé avant le branchement de la base). */
    void persistClientToDb(finalUser, assignVetTeamMember(finalUser.email).name);
    setActiveModal(null);
    router.push("/espace-utilisateur");
  };
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      {/* ── HEADER ── */}
      <header
        className={`header-animated fixed top-0 left-0 w-full z-50 flex justify-between items-center px-4 sm:px-[5%] py-3 sm:py-5 transition-colors duration-300 ${
          scrolled
      ...
      >
        <a
          ...
          className={`logo-link text-2xl sm:text-3xl font-bold flex items-center gap-2 sm:gap-2.5 transition-colors duration-300 cursor-pointer ${
        ...
        <nav className="flex items-center gap-2 sm:gap-6">
          ...
          {adminSession && (
            <Link
              href="/espace-admin"
              title="Retourner à l'espace admin (session toujours active)"
          ...
          <a
            href="#contact"
            className="hidden md:inline-block bg-[#2c8c99] text-white px-5 py-2.5 rounded-full font-medium transition-all duration-300 hover:!-translate-y-1 hover:!bg-[#1f636d] hover:!shadow-[0_15px_35px_rgba(44,140,153,0.4)]"
          >
            Prendre RDV
          </a>
          <button
      ...
      {mobileMenuOpen && (
        ...
        >
          <div
            className="mobile-menu-panel absolute top-0 left-0 w-full max-h-[90vh] overflow-y-auto no-scrollbar bg-white shadow-[0_20px_50px_rgba(0,0,0,0.2)] flex flex-col p-4 sm:p-6 gap-2 rounded-b-[24px]"
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                ...
                className="w-9 h-9 shrink-0 rounded-full bg-[#f4f7f6] text-[#888] text-base flex items-center justify-center transition-all duration-300 hover:!bg-[#2c8c99] hover:!text-white"
              >
      ...
      <section
        id="top"
        className="relative min-h-dvh h-dvh w-full flex items-center justify-start px-5 sm:pl-[8%] text-white overflow-hidden"
        style={{
          backgroundImage:
            "linear-gradient(to right, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.55) 55%, rgba(0,0,0,0.35) 100%), url('https://images.unsplash.com/photo-1548199973-03cce0bbc87b?q=80&w=2070&auto=format&fit=crop')",
          backgroundSize: "cover",
          backgroundPosition: "center",
        }}
      >
        <div className="max-w-[650px] w-full z-10 mt-10 sm:mt-10">
          <h1 className="hero-title text-3xl sm:text-4xl md:text-6xl font-extrabold mb-4 sm:mb-6 leading-[1.1] tracking-tight">
            La santé de votre animal,{" "}
            <span className="text-[#4dd0e1]">notre priorité</span>.
          </h1>
          <p className="hero-text text-sm sm:text-base md:text-lg leading-relaxed font-light opacity-0 mb-6 sm:mb-10">
          ...
          <a
            href="#contact"
            onClick={() => {
              // Sur mobile (pas de vraie souris) : un tap bascule l'état
              // visuel "actif", un second tap le repasse à la normale.
              if (window.matchMedia("(hover: none)").matches) {
                setHeroBtnTapped((v) => !v);
              }
            }}
            className={`hero-btn inline-block text-white px-6 sm:px-10 py-3 sm:py-[18px] rounded-full text-base sm:text-lg font-bold transition-all duration-300 hover:!-translate-y-1 hover:!bg-[#1f636d] hover:!shadow-[0_15px_35px_rgba(44,140,153,0.4)] opacity-0 ${
              heroBtnTapped
                ? "!-translate-y-1 !bg-[#1f636d] !shadow-[0_15px_35px_rgba(44,140,153,0.4)]"
                : "bg-[#2c8c99]"
            }`}
          >
            Prendre Rendez-vous
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-7 max-w-[1200px] mx-auto px-4 sm:px-5">
          {services.map((s, i) => (
            <div
              key={s.title}
            ...
            >
              <div
                onClick={() => {
                  if (window.matchMedia("(hover: none)").matches) {
                    setTappedServiceCard((v) => (v === i ? null : i));
                  }
                }}
                className={`group bg-white p-6 sm:p-10 rounded-[20px] shadow-[0_5px_20px_rgba(0,0,0,0.05)] transition-all duration-500 ease-out hover:[transform:translateY(-1rem)_scale(1.05)] hover:!shadow-[0_15px_40px_rgba(0,0,0,0.1)] cursor-pointer sm:cursor-default ${
                  tappedServiceCard === i
                    ? "[transform:translateY(-1rem)_scale(1.05)] !shadow-[0_15px_40px_rgba(0,0,0,0.1)]"
                    : ""
                }`}
              >
                <div
                  className={`w-16 h-16 sm:w-[90px] sm:h-[90px] bg-[rgba(44,140,153,0.1)] rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6 text-[#2c8c99] text-3xl sm:text-4xl transition-all duration-500 group-hover:!scale-110 group-hover:!bg-[#2c8c99] group-hover:!text-white ${
                    tappedServiceCard === i
                      ? "!scale-110 !bg-[#2c8c99] !text-white"
                      : ""
                  }`}
                >
                  <i className={s.icon} />
                </div>
                <h3 className="text-lg sm:text-xl font-semibold text-[#333] mb-3 sm:mb-4">
                  {s.title}
                </h3>
                <p className="text-sm sm:text-base text-[#666] leading-relaxed">{s.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section
        id="about"
        ref={aboutRef}
        className="about-gradient-bg pb-20 sm:pb-28 relative overflow-hidden"
        style={{ paddingTop: "120px", marginTop: "-80px" }}
      >
        <div className="flex flex-col md:flex-row items-center gap-8 sm:gap-15 max-w-[1200px] mx-auto px-4 sm:px-[5%] relative z-10">
          <div className="animate-on-scroll animate-fade-right flex-1 w-full rounded-[20px] overflow-hidden shadow-[0_20px_40px_rgba(0,0,0,0.15)]">
            <img
              src="https://images.unsplash.com/photo-1628009368231-7bb7cfcb0def?ixlib=rb-4.0.3&auto=format&fit=crop&w=764&q=80"
              alt="Vétérinaire"
              className="w-full h-[280px] sm:h-[450px] object-cover block"
            />
          </div>
          ...
          {team.map((t, i) => (
            ...
            >
              <div
                onClick={() => {
                  if (window.matchMedia("(hover: none)").matches) {
                    setTappedTeamCard((v) => (v === i ? null : i));
                  }
                }}
                className={`group bg-[#f4f7f6] rounded-[20px] overflow-hidden shadow-[0_10px_30px_rgba(0,0,0,0.05)] transition-all duration-500 ease-out hover:[transform:translateY(-0.75rem)_scale(1.05)] pt-8 cursor-pointer sm:cursor-default ${
                  tappedTeamCard === i
                    ? "[transform:translateY(-0.75rem)_scale(1.05)]"
                    : ""
                }`}
      ...
      <section
        id="contact"
        ref={contactRef}
        className="bg-[#f4f7f6] pb-20 sm:pb-28"
        style={{ paddingTop: "120px", marginTop: "-80px" }}
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-12 max-w-[1200px] mx-auto px-4 sm:px-[5%]">
          <div className="animate-on-scroll animate-fade-right">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-[#2c8c99] text-left mb-5">
              Contactez-nous
            </h2>
            <p className="text-[#555] mb-8 text-sm sm:text-base">
              Vous avez une question ou souhaitez prendre rendez-vous ? N’hésitez pas à nous écrire.
            </p>
            <div className="space-y-2 text-[#555] text-sm sm:text-base leading-[2.2]">
              <p>
          ...
          <form
            ...
            className="animate-on-scroll animate-fade-left stagger-2 relative flex flex-col gap-4 sm:gap-5 bg-white p-6 sm:p-8 md:p-10 rounded-[24px] shadow-[0_20px_50px_rgba(0,0,0,0.08)] overflow-hidden"
          >
      ...
      >
        <div className="animate-on-scroll flex justify-center gap-4 mb-6">
          <a
            ...
            aria-label="Facebook"
            className="social-btn w-11 h-11 rounded-full bg-white/10 flex items-center justify-center text-white text-lg transition-all duration-500 hover:!bg-[#2c8c99] hover:!-translate-y-1 active:!bg-[#2c8c99] active:!scale-95"
          >
      ...
      {activeModal && (
        <div
          className="modal-backdrop fixed inset-0 z-[1000] flex items-center justify-center p-3 sm:p-4"
          onClick={() =>
        ...
        >
          <div
            ...
            } max-h-[92vh] sm:max-h-[90vh] bg-white rounded-[20px] sm:rounded-[24px] shadow-[0_25px_60px_rgba(0,0,0,0.25)] overflow-hidden flex flex-col`}
            onClick={(e) => e.stopPropagation()}
          >
